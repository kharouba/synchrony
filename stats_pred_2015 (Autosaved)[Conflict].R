...
env<-merge(unique(yo3[,c("studyid","intid","year")]), unique(mom[,c("studyid","intid","year","envvalue")]), by=c("studyid","intid","year"))
...
m1<-with(sub, lm(envvalue~year))
m3<-lm(envvalue ~ poly(year,2), na.action=na.omit, data=sub) 
