...
[IDEA IS TO RANDOMLY ASSIGN PAIR-WISE INTERACTIONS TO 54 SPECIES, TAKE DIFFERENCE BETWEEN SPECIES, TAKE MEDIAN DIFFERENCE ACROSS INTERACTIONS, REPEAT 999X; problem- 54 unique species but 84 species]
...
df2 <- rawlong[sample(nrow(rawlong)),]; df2$count<-1 # use rawlong BEFORE ALL EDITS not rawlong.nodups because not all interactions have 2 spp
...
sam<-data.frame(array(0, c(109, 2))); 
vec<-c(rep(1:82,1), rep(83:86,2), rep(87:88,3), rep(89:90,4), rep(91,5));  #4 spp repeated 2x, 2 spp repeated 3x; 2 spp repeated 4x, 1 spp repeated 5x
sam$sppid<-vec
