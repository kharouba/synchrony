...
all<-read.csv("spp_phenodata2.csv", header=TRUE, na.strings="<NA>", as.is=TRUE)
int<-read.csv("int_phenodata.csv", header=TRUE, na.strings="NA", as.is=TRUE)
intxn<-read.csv("intxn.csv", header=TRUE, na.strings="<NA>", as.is=TRUE)

#For studies with SAME factors for diff species THEREFORE interaction gets ONE cue
lets<-subset(clim7, studyid!="HMK023" & species=="all" | studyid=="HMK016" | studyid=="HMK030" | studyid=="HMK034" | studyid=="SET005"| studyid=="HMK037")

lio2<-subset(clim7, studyid=="HMK023" & envfactor=="temperature") #added March 2015

lets2<-subset(lets, envfactor=="temperature" | studyid=="HMK016" | studyid=="HMK034")

lets2<-merge(lets[,c("studyid","year","envvalue","z")], int, by=c("studyid","year")); lets2<-na.omit(lets2)
lio3<-rbind(lets2, lio2)
lets2<-lio3
...
mult<-read.csv("clim_multcues.csv", header=TRUE, na.strings="NA", as.is=TRUE)
...
lio<-subset(clim7, studyid=="HMK031" & envfactor=="temperature") #added March 2015
#lets3<-subset(clim7, species=="Epirrita autumnata" | species=="Daphnia3 spp." | species=="Diatom2 spp." | species=="Glis glis")
lets3<-subset(clim7, species=="Epirrita autumnata" | species=="Daphnia3 spp." | species=="Glis glis") #changed March 2015
letsadf<-rbind(lets3, lio) #changed March 2015
lets3<-letsadf #changed March 2015
lets4<-merge(lets3[,c("studyid","year","envvalue","z")], int, by=c("studyid","year")); lets3<-na.omit(lets3)

tot<-rbind(lets, lets3); tot<-na.omit(tot) 
tot<-rbind(lets2, lets3); tot<-na.omit(tot) # use this one for temp sens
...
##extract only march-may daily temp from HMK051
stud<-read.csv("/users/kharouba/google drive/UBC/synchrony project/data/HMK011_dailytemp.csv", header=TRUE, na.strings="NA", as.is=TRUE)

stud2<-subset(stud, newmonth==3 & DAY>=29)
stud3<-subset(stud, newmonth==4)
stud4<-subset(stud, newmonth==5 & DAY<=15)
stud5<-rbind(stud2, stud3, stud4)
...
stud2<-subset(stud, newmonth==3 & DAY>=24)
stud3<-subset(stud, newmonth==4)
stud4<-subset(stud, newmonth==5 & DAY<=10)
stud5<-rbind(stud2, stud3, stud4)

lol4<-with(stud5, aggregate(mean.T, by=list(YEAR), FUN=mean, na.rm=T))
names(lol4)[1]<-"year"; names(lol4)[2]<-"meantemp"
lol4$spp<-"bluetit"

lol5<-rbind(lol,lol2,lol3,lol4)

write.csv(lol5, "/users/kharouba/google drive/UBC/synchrony project/data/HMK051_meantemp.csv")
