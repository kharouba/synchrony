...
# get data
taxer <- read.csv("rough_taxa.csv", header=TRUE)
obsraw <- read.csv("rough.csv", header=TRUE)
...
taxer.trophic <- subset(taxer, interaction != "mutualist" & interaction != "pollination" & interaction != "competition" & studyid != "HMK006" & studyid !="kjb001" & studyid !="kjb002" & studyid != "JEH001")
