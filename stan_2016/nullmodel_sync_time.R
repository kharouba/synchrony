...
means <- aggregate(tog["sync.change"], tog[c("studyid", "intid")], FUN=mean)
names(means)[4]<-"mean_change" #mean of each dataset
sds <- aggregate(tog["sync.change"], tog[c("studyid", "intid")], FUN=sd)
names(sds)[4]<-"sd_change" #sd of each dataset
...
spp1_data$ynull<-sample(a, size=5); #start with a 5 year time series
