...
#Step 4- Create distribution of sync change
sync_mu<--0.052
sync_sd<-0.6
...
timeseries<-c(1,5,10,15,20,25,30)
rowcount2<-1
asdf2<-rowcount2+(54-1) #number of interactions
output<-data.frame(array(0,c(7*54,5)))
...
asdf<-rowcount+(nrow(int_long)-1) #length of NEW time series*2 species

#Step 5- Create new distribution for spp1 from sample mean and sd
...
#Step 6- Create new time-series for spp1 of X length based on normal distribution
...
spp1_datab<-data.frame(array(0,c(nrow(spp1_data),3))) #make new dataframe based on new length
spp1_datab[,1]<-rep(unique(spp1_data$studyid), nrow(spp1_data))
spp1_datab[,2]<-rep(unique(spp1_data$intid), nrow(spp1_data))
spp1_datab[,3]<-rep(unique(spp1_data$species), nrow(spp1_data))

spp1_datab$ynull<-sample(a, size=nrow(spp1_data)); #start with a 5 year time series
spp1_datab$yr1981<-spp1_data$yr1981
#spp1_datab$yr1981<-sample(time2, size=nrow(spp1_data)); #sample X number of years based on length of new time series

#Step 7- Create new distribution for spp2
int_short<-subset(interact, intid==b[i])

obs_sync<-rnorm(1000, (sync_mu*le), (sync_sd*le)); #create distribution of phenovalues based on moean and sd

slope_spp2<-sample(obs_sync, size=1) #draw one mean from means
...
spp2_datab<-data.frame(array(0,c(nrow(spp2_data),3))) #make new dataframe based on new length
spp2_datab[,1]<-rep(unique(spp2_data$studyid), nrow(spp2_data))
spp2_datab[,2]<-rep(unique(spp2_data$intid), nrow(spp2_data))
spp2_datab[,3]<-rep(unique(spp2_data$species), nrow(spp2_data))
spp2_datab$ynull<-sample(bdist, nrow(spp2_data)); #start with a 5 year time series
...
#Step 8- clean to match syncmodels code, gets rid of non-unique data for repeating species within a study (i.e. those species across multiple intxns, e.g. Accipiter nisus) so that models for unique species are built, not unique species-intxns ; Number of unique species should match syncmodels i.e. n=85
