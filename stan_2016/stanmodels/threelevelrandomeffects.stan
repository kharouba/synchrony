...
//Group ids
	int<lower=1> intid[N];			//interaction identity
	int<lower=1> studyLookup[Nspp]; 	//vector of uNque studyids for each species	
...
transformed parameters{
	...
	for (j in 1:Nspp){
		beta_a_spp[j]<-beta_a_study[studyLookup[j]]+a_spp[j]; //Random intercepts;
		beta_b_spp[j]<-beta_b_study[studyLookup[j]]+b_spp[j]; //Random slopes (temperature change);
	}
	
	//Level 1, Individual mean
	for (i in 1:N){
		ypred[i]<-beta_a_spp[intid[i]]+beta_b_spp[intid[i]]*year[i];
	}
}
