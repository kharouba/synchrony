...
for(eachsp in 1:length(uniquespp)){
	lmhere<-lm(envvalue~yr1981, data=subset(clim3, species==uniquespp[eachsp]))
...
plot(range(year), range(y), ylim=c(range(y)), type="n", xlab="Year", ylab="Envvalue", bty="l", main="Raw temperature data")
for (j in 1:Nspp){
...
sigma_b_spp <- mean(goo$sigma_b_spp) # from stan output
...
b <- rnorm(Nspp, mean=mu_b, sd=sigma_b_spp) # this is one way to create fake data from the Stan output to use in the PP check
...
for (j in 1:100){
     b100[,j] <- rnorm(Nspp, mean=mu_b, sd=sigma_b_spp)
}
