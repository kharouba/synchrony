...
plot(range(year), range(y), ylim=c(0, max(y)), type="n", xlab="Year", ylab="Abs(Phenodiff_base)", bty="l", main="Raw phenology data")
for (j in 1:Nspp){
...
b <- abs(rnorm(Nspp, mean=mu_b, sd=sigma_b)) # this is one way to create fake data from the Stan output to use in the PP check
...
par(mar=c(3,3,1,1), mgp=c(1.5,.5,0), tck=-.01)
plot(range(year), range(y), ylim=c(0, max(y)), type="n", xlab="Year", ylab="Abs(phenodiff_base)",   bty="l", main="Data from posterior means")
for (j in 1:Nspp){
   lines(year[species==j], y[species==j])
}
