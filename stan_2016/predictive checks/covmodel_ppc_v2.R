# Posterior Predictive checks for covariate model (e.g. pheno change~ temp change); BELOW FOR SYNC CHANGE
...
#######
FOR SYNC CHANGE~TEMP CHANGE

#for sync:
comppool<-lm(abs(sync.change)~temp.change, data=tdata)
# No pooling:
uniquespp<-unique(tdata$id)
#uniquespp<-unique(rawlong.tot$newid)
slopefits<-rep(1< length(uniquespp))
varfits<-rep(1< length(uniquespp))
intfits<-rep(1< length(uniquespp))
for(eachsp in 1:length(uniquespp)){
	lmhere<-lm(abs(sync.change)~temp.change, data=subset(tdata, id==uniquespp[eachsp]))
	slopefits[eachsp]<-coef(lmhere)[2]
	varfits[eachsp]<-(summary(lmhere)$sigma)**2
	intfits[eachsp]<-coef(lmhere)[1]
}
...
option 1:
par(mar=c(3,3,1,1), mgp=c(1.5,.5,0), tck=-.01)
plot(range(year), range(y), ylim=range(y), type="n", xlab="Temp change", ylab="abs(Sync change)", bty="l", main="No pooling- linear models")
for (j in 1:Nspp){
  abline(lm((y[species==j])~year[species==j]))
}
...
option 2:
par(mar=c(3,3,1,1), mgp=c(1.5,.5,0), tck=-.01)
plot(range(year), range(y), ylim=range(y), type="n", xlab="Temp change", ylab="Sync change", bty="l", main="No pooling intercept, complete pooling slope")
for (j in 1:Nspp){
	m1<-lm((y[species==j])~year[species==j])
  abline(a=coef(m1)[1], b=coef(comppool)[2])
}
...
#Plot the data and see what the raw data predicted from the model looks like
par(mar=c(3,3,1,1), mgp=c(1.5,.5,0), tck=-.01)
plot(range(year), range(y), ylim=range(y), type="n", xlab="Temp change", ylab="abs(sync change)",   bty="l", main="Data from posterior means")
for (j in 1:Nspp){
   abline(a=(y[species==j]), b=mu_b)
}
